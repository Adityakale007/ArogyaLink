  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.timeTextBubble, isUser ? { color: 'rgba(255,255,255,0.7)' } : { color: theme.textSecondary }]}>{message.time}</Text>
      </View>
    </View>
  );
};
...
const DoctorChat = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (user?._id) {
      ...
      socketInstance.on('receive_message', (data) => {
        if (data.message && (data.senderId === doctor.id || data.message.senderId === doctor.id)) {
          const formattedMessage = {
            id: data.message._id,
            text: data.message.message,
            isUser: data.message.senderId === user._id,
      ...
      if (response.ok) {
        ...
        if (data.success) {
          const formattedMessages = data.messages.map(msg => ({
            id: msg._id,
            text: msg.message,
            isUser: msg.senderId._id === user._id,
  ...
  const handleSend = () => {
    if (inputText.trim() && user?._id && doctor.id) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <StatusBar backgroundColor={theme.headerBackground} barStyle={theme.statusBarStyle} />
      ...
      <View style={[styles.header, { backgroundColor: theme.primary }]}>
        ...
        <TouchableOpacity onPress={handleAppointmentStart} style={styles.appointmentButton}>
