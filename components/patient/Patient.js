...
const usePatientData = () => {
  ...
  useEffect(() => {
    const fetchPatientName = async () => {
      try {
        // First try to get name from user context (already loaded from login)
        if (user?.name) {
          setName(user.name);
          setLoading(false);
          return;
        ...
        // If user ID is available but name is not, fetch from API
        if (user?._id) {
        ...
        setName('Patient'); // Fallback name on error
      } finally {
        setLoading(false);
      }
    };
  ...
  // Return the necessary states
  return { patientName: name, loading, error };
};

// module.exports = usePatientData; 
// Note: In React Native, you typically use export default or named exports
...
const Patient = () => {
  ...
  // *** CHANGE: Use the custom hook to fetch the patient name ***
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={[styles.container, { paddingTop: insets.top + 10, backgroundColor: theme.background }]}>
        <View 
        ...
        >
          ...
          >
            ...
            {/* *** CHANGE: Use patientName state variable in the greeting *** */}
            <Text style={[styles.greeting, { color: theme.text }]}>Hi, {patientName}</Text> 
        ...
        <TouchableOpacity
          style={[styles.pharmaFab, styles.fabShadow, { 
        ...
        <TouchableOpacity
          style={[styles.fab, styles.fabShadow, { 
            bottom: Math.max(38, insets.bottom + 60 + 10), 
            right: 24,
            backgroundColor: theme.darkBlue || COLORS.darkBlue,
            zIndex: 1000,
          }]}
...
const styles = StyleSheet.create({
  ...
  fab: { position: 'absolute', width: 56, height: 56, borderRadius: 28, justifyContent: 'center', alignItems: 'center', },
